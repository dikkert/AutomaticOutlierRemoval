...
class extractandTrain:
    def extract_train(self,directory,batch_size=10000):
        ...
        for pathname in self.pathnames:
            ...
            # eigenvalues_sum = np.array(pc["Eigenvalues__sum__(0.01)"])
            # omnivariance = np.array(pc["Omnivariance__(0.01)"])
            # eigentropy = np.array(pc["Eigenentropy__(0.01)"])
            # anisotropy = np.array(pc['Anisotropy__(0.01)'])
            # planarity = np.array(pc["Planarity__(0.01)"])
            # linearity = np.array(pc["Linearity__(0.01)"])
            # pca1 = np.array(pc["PCA1__(0.01)"])
            # pca2 = np.array(pc["PCA2__(0.01)"])
            # surface_variation = np.array(pc["Surface__variation__(0.01)"])
            # sphericity = np.array(pc["Sphericity__(0.01)"])
            # verticality = np.array(pc["Verticality__(0.01)"])
            # eigen1 = np.array(pc["1st__eigenvalue__(0.01)"])
            # eigen2 = np.array(pc["2nd__eigenvalue__(0.01)"])
            # eigen3 = np.array(pc["3rd__eigenvalue__(0.01)"])
            density = np.array(pc["Volume__density__(r_eq_0.05)"])
            print("extracted all features")
            # load features on a numpy array
            features = np.c_[R,G,B,Intensity, density]
    ...
    def ocsvmtrainer(self):
        ...
        # define the method
        self.ocsvm= SGDOneClassSVM(nu=0.8, average= True)
...
def ocsvmpredict(dir_path,out_path):
        ...
        for filename in os.listdir(dir_path):
            if not os.path.isdir(filename):
                if filename.endswith(".las") and not filename.endswith(("_outliers.laz", "outliers.las")):
                    path = os.path.join(dir_path,filename).replace("\\","/")
...
with open('coef_density_large_train.csv', mode='w') as file:
